    ...
    public void extendBlocks(){
        switch(this.state){
            case O:
                ...
                this.blocks[2].position = this.p.right().right();
                this.blocks[3].position = this.p.right().right().up();
                break;
            case R:
                this.blocks[0].position = this.p;
                this.blocks[1].position = this.p.down();
                this.blocks[2].position = this.p.down().down();
                this.blocks[3].position = this.p.down().down().right();
                break;
            case D:
                this.blocks[0].position = this.p;
                this.blocks[1].position = this.p.left();
                this.blocks[2].position = this.p.left().left();
                this.blocks[3].position = this.p.left().left().down();
                break;
            case L:
                this.blocks[0].position = this.p;
                this.blocks[1].position = this.p.up();
                this.blocks[2].position = this.p.up().up();
                this.blocks[3].position = this.p.up().up().left();
        }
    }

    @Override
    public Shape cloneShape(){
        Shape_L result = new Shape_L();
